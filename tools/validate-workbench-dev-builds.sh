...
fs.writeFileSync('dist/workbench-identity.json', `${JSON.stringify(evidence, null, 2)}\n`);
fs.writeFileSync('dist-mv3/workbench-identity.json', `${JSON.stringify(evidence, null, 2)}\n`);
